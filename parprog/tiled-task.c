// assignment2 OpenMP tester - OpenMP tasks code, tiled version
// compile with:  gcc -Wall -O2 -fopenmp tiled-task.c -o tiled-task -DN=4000 -DTS=200
...
double *a;
  ...
  for (int ii=1;ii<N;ii+=TS) {		// starting row of tile
    ...
    for (int jj=1;jj<N;jj+=TS) {	// starting column of tile
      ...
      #pragma omp task depend(out:a[istart*N+jstart]) \
                           depend(in:a[(istart-TS)*N+jstart-TS], \
                                     a[(istart-TS)*N+jstart],a[istart*N+jstart-TS])
      {
