...
// Each block performs a tree reduction of the partial sums of its threads
// NOTE: this version is inefficient due to shared memory bank conflicts.

// Compile with: nvcc sum-reduction-float-block-reduction2.cu -o sum-reduction-float-block-reduction2 -DN=10000000
...
__global__ void sumReduction(float *a,float *psums) {
  ...
  // block reduction phase in shared memory (THREADS partial sums -> 1 result)
  ...
  // write final result of this block to global memory
  if (threadIdx.x==0) {
    psums[blockIdx.x] = buffer[0];
  }
}
