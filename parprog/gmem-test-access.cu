...
// Compile with: nvcc gmem-test-access.cu -o gmem-test-access -DN=25600000

// NOTE: array size is rounded up to 256 (= grid size).
...
int main() {
  ...
  int ceil_n = threads*blocks;
  
  printf("Array elements: %d (%d threads x %d blocks)\n",ceil_n,threads,blocks);
