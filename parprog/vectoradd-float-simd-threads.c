...
// compile with: gcc -msse2 -O2 -Wall -fopenmp vectoradd-float-simd-threads.c -o vectoradd-float-simd-threads -DN=10000 -DR=10000
...
int i;
  ...
  #pragma omp parallel
  {
    ...
    for (int j=0;j<R;j++) {
      ...
      #pragma omp for simd aligned(a,b,c:16) nowait  
      for (int i=0;i<N;i++) {
        a[i] = b[i]+c[i];
      }
    }
