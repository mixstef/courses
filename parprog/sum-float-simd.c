// sum reduction of an array of floats using OpenMP SIMD construct
// compile with: gcc -msse2 -Wall -O2 -fopenmp sum-float-simd.c -o sum-float-simd -DN=10000 -DR=100000
...
int main() {
  ...
  for (int j=0;j<R;j++) {
    ...
    #pragma omp simd aligned(a:16) reduction(+:fsum)
    for (int i=0;i<N;i++) {
      fsum += a[i];
    }
  }
